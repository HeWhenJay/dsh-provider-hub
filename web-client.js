        ...
        return React.createElement('article', { className: 'ph-specRow', 'data-status': statusKey, key: model.id },
          ...
          React.createElement('div', { className: 'ph-specGrid' },
            ...
            React.createElement('div', { className: 'ph-specMetric' }, '思考程度', React.createElement('strong', null, reasoningText(model.reasoningEfforts)), evidenceLabel(model, 'reasoningEfforts') && React.createElement('span', { className: 'ph-meta' }, evidenceLabel(model, 'reasoningEfforts'))),
             React.createElement('div', { className: 'ph-specMetric' }, '参考单价', React.createElement('strong', null, model.modelPricing ? Object.entries(model.modelPricing).filter(([key]) => key !== 'currency').map(([key, value]) => `${key.replace('PerMillion', '')} ${value}`).join(' · ') : '未配置'), model.modelPricing?.currency && React.createElement('span', { className: 'ph-meta' }, `${model.modelPricing.currency} · 规格参考价`))
          ),
          React.createElement('div', { className: 'ph-specTail' }, model.error && React.createElement('div', { className: 'ph-specError' }, researchErrorText(model.error)), model.diagnostics && React.createElement('span', { className: 'ph-muted' }, `查询 ${model.diagnostics.successfulSearches || 0}/${model.diagnostics.queryCount || 0} · 候选 ${model.diagnostics.selectedSources || 0} · 有证据 ${model.diagnostics.evidenceSources || 0} · 模型调用 ${model.diagnostics.modelCallCompleted ? '已完成' : model.diagnostics.modelCallStarted ? '进行过' : '未开始'}`), !model.sources?.length && !model.error && React.createElement('span', { className: 'ph-muted' }, '等待可验证来源')),
