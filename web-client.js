window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Specifications({ hub }) {
      ...
      return React.createElement(React.Fragment, null,
        React.createElement('div', { className: 'ph-sectionTitle' }, React.createElement('div', null, React.createElement('h3', null, '模型规格'), React.createElement('div', { className: 'ph-help' }, `从厂商官方来源逐字段补全；查到的写入，未查到的保持空白。研究调用：${research?.selection ? `${research.selection.provider} / ${research.selection.model}` : '等待 DSH 默认模型'}`)), React.createElement('div', { className: 'ph-actions' }, retryable && React.createElement(Button, { variant: 'outline', size: 'sm', disabled: running || !research?.available, icon: React.createElement(IconRefreshOutline16, { size: 16 }), onClick: retry }, '重新补全规格'))),
