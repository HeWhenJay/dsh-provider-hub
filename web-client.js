window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Channels({ hub }) {
      ...
      const researchText = runningResearch ? `正在补全 ${research.completed || 0}/${research.total || 0}${research.currentModel ? `：${research.currentModel}` : ''}` : research?.phase === 'done' ? `已补全 ${research.updated || 0} 个，跳过 ${research.skipped || 0} 个` : research?.phase === 'error' ? `补全失败：${research.error || `${research.failed || 0} 个模型失败`}` : '从官方来源补全上下文、最大输出与思考强度';
      return React.createElement(React.Fragment, null,
        React.createElement('div', { className: 'ph-sectionTitle' }, React.createElement('div', null, React.createElement('h3', null, '供应商与 API 渠道'), React.createElement('div', { className: 'ph-help' }, '高优先级渠道先用，保底渠道最后使用；模型可一键发现。')), React.createElement('div', { className: 'ph-actions' }, React.createElement(Button, { variant: 'outline', size: 'sm', disabled: runningResearch || !research?.available || !(research?.models?.length), icon: runningResearch ? React.createElement(IconLoadingOutline16, { size: 16, className: 'ph-spin' }) : React.createElement(IconRefreshOutline16, { size: 16 }), onClick: startResearch }, runningResearch ? '补全中…' : '补全模型规格'), React.createElement(Button, { variant: 'primary', size: 'sm', icon: React.createElement(IconPlusOutline16, { size: 16 }), onClick: () => setEditing(false) }, '添加供应商'))),
        React.createElement('div', { className: 'ph-discovery' }, React.createElement('div', { className: 'ph-discoveryText' }, React.createElement('strong', null, '官方模型规格'), React.createElement('br'), researchText), research?.sources?.length ? React.createElement('div', { className: 'ph-models' }, research.sources.slice(0, 3).map((source) => React.createElement('a', { className: 'ph-model', href: source, target: '_blank', rel: 'noreferrer', key: source }, new URL(source).hostname)), research.sources.length > 3 && React.createElement('span', { className: 'ph-model' }, `+${research.sources.length - 3}`)) : null),
