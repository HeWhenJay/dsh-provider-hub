window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const emptyRoute = { id: '', displayName: '', keyName: '', baseURL: '', api: 'openai-completions', apiKeyEnv: '', apiKeyEnvManual: false, apiKey: '', priority: 100, backup: false, modelsText: '', modelAllowlistText: '', discoveredModels: [], modelAliasesText: '{}' };
    ...
    function researchErrorText(error) {
      ...
      if (/no official source/i.test(value)) return '未找到可接受的官方来源';
      if (/did not prove any supported specification field/i.test(value)) return '官方证据未证明可用的规格字段';
    ...
    function ModelSpecifications({ research }) {
      ...
      return React.createElement('div', { className: 'ph-specList' }, models.map((model) => {
        ...
        return React.createElement('article', { className: 'ph-specRow', 'data-status': statusKey, key: model.id },
          React.createElement('div', { className: 'ph-specTop' }, React.createElement('div', null, React.createElement('div', { className: 'ph-specId' }, model.name || model.id), model.name && React.createElement('div', { className: 'ph-meta' }, model.id)), React.createElement('span', { className: 'ph-model', style: { color: statusKey === 'configured' ? '#168755' : statusKey === 'partial' ? '#a87200' : statusKey === 'skipped' ? '#bd570f' : statusKey === 'failed' ? '#c92f3c' : undefined, fontWeight: 650 } }, status)),
          React.createElement('div', { className: 'ph-specGrid' },
            React.createElement('div', { className: 'ph-specMetric' }, '上下文窗口', React.createElement('strong', null, formatCount(model.contextWindow))),
            React.createElement('div', { className: 'ph-specMetric' }, '最大输出窗口', React.createElement('strong', null, formatCount(model.maxTokens))),
            React.createElement('div', { className: 'ph-specMetric' }, '思考程度', React.createElement('strong', null, reasoningText(model.reasoningEfforts)))
          ),
          React.createElement('div', { className: 'ph-specTail' }, model.error && React.createElement('div', { className: 'ph-specError' }, researchErrorText(model.error)), model.sources?.length ? React.createElement('div', { className: 'ph-specSources' }, model.sources.map((source) => React.createElement('a', { className: 'ph-model', href: source, target: '_blank', rel: 'noreferrer', key: source }, new URL(source).hostname))) : !model.error && React.createElement('span', { className: 'ph-muted' }, '等待官方来源'))
        );
      }));
    }
    ...
    function Specifications({ hub }) {
      ...
      const retry = async () => { hub.setError(''); try { const next = await hubApi('/models/research', { method: 'POST', body: '{}' }); setResearch(next.research); } catch (error) { hub.setError(error.message); } };
      const models = research?.models || [];
      ...
      return React.createElement(React.Fragment, null,
        React.createElement('div', { className: 'ph-sectionTitle' }, React.createElement('div', null, React.createElement('h3', null, '模型规格'), React.createElement('div', { className: 'ph-help' }, '从厂商官方来源逐字段补全；查到的写入，未查到的保持空白。')), React.createElement('div', { className: 'ph-actions' }, retryable && React.createElement(Button, { variant: 'outline', size: 'sm', disabled: running || !research?.available, icon: React.createElement(IconRefreshOutline16, { size: 16 }), onClick: retry }, '重新补全规格'))),
    ...
    function Logs({ hub }) {
      ...
      return React.createElement(React.Fragment, null,
        React.createElement('div', { className: 'ph-sectionTitle' }, React.createElement('div', null, React.createElement('h3', null, '请求日志'), React.createElement('div', { className: 'ph-help' }, '显示 API Key 名称、模型、状态和延迟；不记录密钥值、凭据引用、提示词或完整 URL。')), React.createElement(Button, { variant: 'ghost', size: 'sm', onClick: clear }, '清空')),
        hub.logs.length === 0 ? React.createElement('div', { className: 'ph-empty' }, '暂无请求日志。') : React.createElement('div', null, hub.logs.map((log) => React.createElement('div', { className: 'ph-log', key: log.id }, React.createElement('span', { className: 'ph-logTime' }, new Date(log.time).toLocaleTimeString()), React.createElement('span', null, log.model), React.createElement('span', { className: 'ph-logRoute' }, log.keyName || log.routeName), React.createElement('span', { style: { color: log.ok ? 'var(--dsw-alias-state-success-primary)' : 'var(--dsw-alias-state-error-primary)' } }, log.status || 'ERR'), React.createElement('span', { className: 'ph-logLatency ph-muted' }, `${log.latencyMs}ms`))))
      );
    }
